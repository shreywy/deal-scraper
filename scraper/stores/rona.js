...
const STORE_NAME = 'RONA';
const STORE_KEY = 'rona';
const CURRENCY = 'CAD';
...
async function scrape(browser, onProgress = () => {}) {
  ...
  try {
    onProgress('RONA: trying XHR interception…');
    const xhrDeals = await tryXhrInterception(browser, onProgress);
    if (xhrDeals && xhrDeals.length > 0) {
      deals.push(...xhrDeals);
      onProgress(`RONA: found ${deals.length} deals via XHR`);
      return deals;
    }

    // Fallback to DOM if XHR fails
    onProgress('RONA: trying DOM scraping…');
    const domDeals = await tryDomScraping(browser, onProgress);
    if (domDeals && domDeals.length > 0) {
      deals.push(...domDeals);
      onProgress(`RONA: found ${deals.length} deals via DOM`);
      return deals;
    }

    onProgress('RONA: no deals found');
    return deals;

  } catch (error) {
    onProgress(`RONA: error — ${error.message}`);
...
async function tryXhrInterception(browser, onProgress) {
  ...
  try {
        ...
        if (contentType.includes('json') &&
          ...
          try {
            ...
            if (Array.isArray(products) && products.length > 0) {
              ...
              onProgress(`RONA XHR: intercepted ${products.length} products (total: ${interceptedProducts.length})`);
            }
          } catch (jsonError) {
    ...
    await page.goto('https://www.rona.ca/en/specials', {
...
/**
 * Parse RONA products from API response
 */

...
const STORE_NAME = 'Factory Direct';
const STORE_KEY = 'factorydirect';
const CURRENCY = 'CAD';
...
/**
 * Scrapes deals from Factory Direct
...
async function scrape(browser, onProgress = () => {}) {
  ...
  try {
    onProgress('Factory Direct: trying Shopify API…');
    const apiDeals = await tryShopifyAPI(onProgress);
    if (apiDeals && apiDeals.length > 0) {
      deals.push(...apiDeals);
      onProgress(`Factory Direct: found ${deals.length} deals via Shopify API`);
      return deals;
    }

    // Fallback to DOM scraping
    onProgress('Factory Direct: trying DOM scraping…');
    const domDeals = await tryDomScraping(browser, onProgress);
    if (domDeals && domDeals.length > 0) {
      deals.push(...domDeals);
      onProgress(`Factory Direct: found ${deals.length} deals via DOM`);
      return deals;
    }

    onProgress('Factory Direct: no deals found');
    return deals;

  } catch (error) {
    onProgress(`Factory Direct: error — ${error.message}`);
...
async function tryShopifyAPI(onProgress) {
  ...
  try {
    ...
    while (hasMore && page <= 10) {
      const url = `https://www.factorydirect.ca/collections/on-sale/products.json?limit=250&page=${page}`;

      onProgress(`Factory Direct API: fetching page ${page}…`);
      ...
      if (!response.ok) {
        onProgress(`Factory Direct API: returned ${response.status}`);
        break;
      }
      ...
      for (const product of products) {
        try {
          ...
          for (const variant of variants) {
            ...
            const deal = {
              id: slugify(`factorydirect-${name}`),
              store: STORE_NAME,
              storeKey: STORE_KEY,
              name,
              url: `https://www.factorydirect.ca/products/${product.handle}`,
      ...
      onProgress(`Factory Direct API: page ${page} — ${deals.length} deals so far`);
      page++;
    }
    ...
    onProgress(`Factory Direct API: error — ${error.message}`);
    return null;
  }
}
...
async function tryDomScraping(browser, onProgress) {
  ...
  try {
    ...
    await page.goto('https://www.factorydirect.ca/pages/flyer-deals', {
    ...
    for (const product of products) {
      try {
        ...
        const deal = {
          id: slugify(`factorydirect-${product.name}`),
          store: STORE_NAME,
          storeKey: STORE_KEY,
          name: product.name,
          url: product.url,

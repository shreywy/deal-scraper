...
const STORE_NAME = 'RVCA';
const STORE_KEY = 'rvca';
const CURRENCY = 'USD';
...
/**
 * RVCA - Surf/skate lifestyle brand (Shopify)
 *
...
async function scrape(_browser, onProgress = () => {}) {
  onProgress('RVCA: fetching USD→CAD rate…');
  const rate = await getUSDtoCAD();
  onProgress(`RVCA: 1 USD = ${rate.toFixed(4)} CAD`);

  const allDeals = [];
  const seen = new Set();

  for (const { slug, gender } of SALE_COLLECTIONS) {
    onProgress(`RVCA: fetching ${gender} sale products from Shopify API…`);

    let page = 1;
    let hasMore = true;

    while (hasMore) {
      const url = `https://www.rvca.com/collections/${slug}/products.json?limit=250&page=${page}`;
      ...
      try {
        ...
        if (!res.ok) {
          onProgress(`RVCA: HTTP ${res.status} on ${slug} page ${page}`);
          break;
        }
        ...
        onProgress(`RVCA: ${gender} - ${allDeals.filter(d => d.gender === gender).length} deals (page ${page})`);

        if (products.length < 250) hasMore = false;
        page++;
      } catch (err) {
        onProgress(`RVCA: Error on ${slug} page ${page}: ${err.message}`);
        break;
      }
    }
  }

  onProgress(`RVCA: total ${allDeals.length} deals found`);
  return allDeals;
}
...
function mapShopifyProduct(p, rate, gender, seen) {
  try {
    ...
    const url = `https://www.rvca.com/products/${handle}`;

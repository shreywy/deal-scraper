...
/**
 * Scrapes deals from Visions Electronics
...
async function scrape(browser, onProgress = () => {}) {
  ...
  try {
    onProgress('Visions Electronics: navigating to promotions…');
    ...
    await page.goto('https://www.visions.ca/Catalogue/Section/Promotions/1', {
    ...
    onProgress('Visions Electronics: extracting products…');
    ...
    const products = await page.evaluate(() => {
      ...
      return cards.map(card => {
        try {
          ...
          // Find URL
          const linkEl = card.querySelector('a[href*="/product/"], a[href*="/catalogue/"], a');
    ...
    onProgress(`Visions Electronics: found ${products.length} products with prices`);
    ...
    for (const product of products) {
      try {
        ...
        const deal = {
          id: slugify(`visions-${product.name}`),
          store: STORE_NAME,
          storeKey: STORE_KEY,
          name: product.name,
          url: product.url,
    ...
    onProgress(`Visions Electronics: found ${deals.length} deals`);
    return deals;

  } catch (error) {
    onProgress(`Visions Electronics: error — ${error.message}`);

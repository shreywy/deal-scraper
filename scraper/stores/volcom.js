...
const STORE_NAME = 'Volcom';
const STORE_KEY = 'volcom';
const CURRENCY = 'USD';
...
/**
 * Volcom - Skate/surf streetwear (Shopify)
 *
...
async function scrape(_browser, onProgress = () => {}) {
  onProgress('Volcom: fetching USD→CAD rate…');
  const rate = await getUSDtoCAD();
  onProgress(`Volcom: 1 USD = ${rate.toFixed(4)} CAD`);

  const allDeals = [];
  const seen = new Set();

  for (const { slug, gender } of SALE_COLLECTIONS) {
    onProgress(`Volcom: fetching ${gender} sale products from Shopify API…`);

    let page = 1;
    let hasMore = true;

    while (hasMore) {
      const url = `https://www.volcom.com/collections/${slug}/products.json?limit=250&page=${page}`;
      ...
      try {
        ...
        if (!res.ok) {
          onProgress(`Volcom: HTTP ${res.status} on ${slug} page ${page}`);
          break;
        }
        ...
        onProgress(`Volcom: ${gender} - ${allDeals.filter(d => d.gender === gender).length} deals (page ${page})`);

        if (products.length < 250) hasMore = false;
        page++;
      } catch (err) {
        onProgress(`Volcom: Error on ${slug} page ${page}: ${err.message}`);
        break;
      }
    }
  }

  onProgress(`Volcom: total ${allDeals.length} deals found`);
  return allDeals;
}
...
function mapShopifyProduct(p, rate, gender, seen) {
  try {
    ...
    const url = `https://www.volcom.com/products/${handle}`;

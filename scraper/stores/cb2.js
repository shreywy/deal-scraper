...
async function scrape(browser, onProgress = () => {}) {
  ...
  try {
    onProgress('CB2 Canada: trying XHR interception…');
    const xhrDeals = await tryXhrInterception(browser, onProgress);
    if (xhrDeals && xhrDeals.length > 0) {
      deals.push(...xhrDeals);
      onProgress(`CB2 Canada: found ${deals.length} deals via XHR`);
      return deals;
    }

    // Fallback to DOM if XHR fails
    onProgress('CB2 Canada: trying DOM scraping…');
    const domDeals = await tryDomScraping(browser, onProgress);
    if (domDeals && domDeals.length > 0) {
      deals.push(...domDeals);
      onProgress(`CB2 Canada: found ${deals.length} deals via DOM`);
      return deals;
    }

    onProgress('CB2 Canada: no deals found');
    return deals;

  } catch (error) {
    onProgress(`CB2 Canada: error — ${error.message}`);
...
async function tryXhrInterception(browser, onProgress) {
  ...
  try {
        ...
        if (contentType.includes('json') &&
          ...
          try {
            ...
            if (Array.isArray(products) && products.length > 0) {
              ...
              onProgress(`CB2 Canada XHR: intercepted ${products.length} products (total: ${interceptedProducts.length})`);
            }
          } catch (jsonError) {
    ...
    await page.goto('https://www.cb2.ca/sale', {
...
/**
 * Parse CB2 products from API response
 */

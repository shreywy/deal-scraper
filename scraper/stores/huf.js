...
async function scrape(_browser, onProgress = () => {}) {
  onProgress('HUF: fetching USD→CAD rate…');
  const rate = await getUSDtoCAD();
  onProgress(`HUF: 1 USD = ${rate.toFixed(4)} CAD`);

  const allDeals = [];
  const seen = new Set();

  for (const { slug, gender } of SALE_COLLECTIONS) {
    onProgress(`HUF: fetching ${gender} sale products from Shopify API…`);

    let page = 1;
    let hasMore = true;

    while (hasMore) {
      const url = `https://www.hufworldwide.com/collections/${slug}/products.json?limit=250&page=${page}`;
      ...
      try {
        ...
        if (!res.ok) {
          onProgress(`HUF: HTTP ${res.status} on ${slug} page ${page}`);
          break;
        }
        ...
        onProgress(`HUF: ${gender} - ${allDeals.filter(d => d.gender === gender).length} deals (page ${page})`);

        if (products.length < 250) hasMore = false;
        page++;
      } catch (err) {
        onProgress(`HUF: Error on ${slug} page ${page}: ${err.message}`);
        break;
      }
    }
  }

  onProgress(`HUF: total ${allDeals.length} deals found`);
  return allDeals;
}
...
function mapShopifyProduct(p, rate, gender, seen) {
  try {
    ...
    const url = `https://www.hufworldwide.com/products/${handle}`;

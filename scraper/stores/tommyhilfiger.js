...
const STORE_NAME = 'Tommy Hilfiger';
const STORE_KEY = 'tommyhilfiger';
const CURRENCY = 'CAD';
...
/**
 * Tommy Hilfiger Canada — SFCC platform, XHR intercept + DOM fallback
 *
...
async function scrape(browser, onProgress = () => {}) {
  ...
  onProgress('Tommy Hilfiger: loading sale pages...');
  ...
  for (const saleUrl of SALE_URLS) {
    try {
      onProgress(`Tommy Hilfiger: trying ${saleUrl}...`);
      ...
      if (response && (response.status() === 403 || response.status() === 404 || response.status() === 503)) {
        onProgress(`Tommy Hilfiger: ${saleUrl} returned ${response.status()}`);
        continue;
      }
      ...
      if (rawProducts.length > 0) {
        onProgress(`Tommy Hilfiger: processing ${rawProducts.length} API products...`);
      ...
      if (allDeals.length === 0) {
        onProgress('Tommy Hilfiger: trying DOM scraping...');
      ...
      onProgress(`Tommy Hilfiger: error on ${saleUrl} — ${err.message}`);
    }
  }
  ...
  onProgress(`Tommy Hilfiger: found ${allDeals.length} deals`);
  return allDeals;
}
...
function mapProduct(p, seen) {
  try {
    ...
    const url = p.pdpUrl || p.url || (id ? `https://www.tommy.com/en-ca/p/${id}` : '');

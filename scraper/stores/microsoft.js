...
async function scrape(browser, onProgress = () => {}) {
  ...
  // Intercept XHR/API responses - Microsoft uses React with API calls
  page.on('response', async response => {
    ...
    try {
      ...
      for (const p of arr) {
        const id = p?.id || p?.productId || p?.ProductId || p?.sku || p?.handle;
  ...
  try {
    onProgress('Microsoft CA: navigating to sale page...');
    await page.goto('https://www.microsoft.com/en-ca/store/b/sale', {
    ...
    if (intercepted.length > 0) {
      ...
      onProgress(`Microsoft CA: found ${deals.length} deals (API)`);
      return deals;
    }

    // DOM fallback
    onProgress('Microsoft CA: trying DOM scrape...');
    const rawDeals = await page.evaluate(({ storeName, storeKey }) => {
      const cards = document.querySelectorAll('.c-product-card, [class*="product-card"], [class*="Product"], .product, article');
      ...
      return [...cards].map(card => {
        ...
        const name = (
          card.querySelector('.c-product-card-title, [class*="title"], [class*="name"], h2, h3, h4')?.textContent ||
        ...
        const priceEls = card.querySelectorAll('.c-price, .c-price-original, [class*="price"], [class*="Price"]');
        let price = null;
        ...
        for (const el of priceEls) {
          ...
          if (cls.includes('sale') || cls.includes('Sale') || cls.includes('current') || cls.includes('Current') ||
              cls.includes('c-price') && !cls.includes('original')) {
    ...
    onProgress(`Microsoft CA: found ${tagged.length} deals (DOM)`);
    return tagged;

  } catch (error) {
    onProgress(`Microsoft CA: error - ${error.message}`);
    return [];
  } finally {
    await context.close();
  }

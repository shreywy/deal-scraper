...
const STORE_NAME = 'Home Depot CA';
const STORE_KEY = 'homedepot';
const CURRENCY = 'CAD';
...
/**
 * Scrapes deals from Home Depot Canada
...
async function scrape(browser, onProgress = () => {}) {
  ...
  try {
    onProgress('Home Depot CA: trying XHR interception…');
    const xhrDeals = await tryXhrInterception(browser, onProgress);
    if (xhrDeals && xhrDeals.length > 0) {
      deals.push(...xhrDeals);
      onProgress(`Home Depot CA: found ${deals.length} deals via XHR`);
      return deals;
    }

    // Fallback to DOM if XHR fails
    onProgress('Home Depot CA: trying DOM scraping…');
    const domDeals = await tryDomScraping(browser, onProgress);
    if (domDeals && domDeals.length > 0) {
      deals.push(...domDeals);
      onProgress(`Home Depot CA: found ${deals.length} deals via DOM`);
      return deals;
    }

    onProgress('Home Depot CA: no deals found');
    return deals;

  } catch (error) {
    onProgress(`Home Depot CA: error — ${error.message}`);
...
async function tryXhrInterception(browser, onProgress) {
  ...
  try {
        ...
        if (contentType.includes('json') &&
          ...
          try {
            ...
            if (Array.isArray(products) && products.length > 0) {
              ...
              onProgress(`Home Depot CA XHR: intercepted ${products.length} products (total: ${interceptedProducts.length})`);
            }
          } catch (jsonError) {
    ...
    await page.goto('https://www.homedepot.ca/en/home/specials/todays-specials.html', {
  ...
  try {
    ...
    for (const product of products) {
      try {
        ...
        const deal = {
          id: slugify(`homedepot-${product.name}`),
          store: STORE_NAME,
          storeKey: STORE_KEY,
          name: product.name,
          url: product.url,
...
/**
 * Parse Home Depot products from API response
 */
  ...
  for (const product of products) {
    try {
      ...
      if (url && !url.startsWith('http')) {
        url = `https://www.homedepot.ca${url}`;
      }
      if (!url) continue;
      ...
      if (image && !image.startsWith('http')) {
        image = `https://www.homedepot.ca${image}`;
      }

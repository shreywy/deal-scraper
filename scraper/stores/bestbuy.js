...
const STORE_NAME = 'Best Buy CA';
const STORE_KEY = 'bestbuy';
const CURRENCY = 'CAD';
...
/**
 * Scrapes deals from Best Buy Canada
...
async function tryXhrInterception(browser, onProgress) {
  ...
  try {
        ...
        if (contentType.includes('json') &&
          ...
          try {
            ...
            if (Array.isArray(products) && products.length > 0) {
              ...
              onProgress(`Best Buy CA XHR: intercepted ${products.length} products (total: ${interceptedProducts.length})`);
            }
          } catch (jsonError) {
    ...
    // Navigate to deals pages
    const urls = [
      'https://www.bestbuy.ca/en-ca/deals',
      'https://www.bestbuy.ca/en-ca/brand/clearance',
    ];
    ...
    for (const url of urls) {
      try {
        ...
        onProgress(`Best Buy CA: failed to load ${url} — ${navError.message}`);
      }
    }
...
async function tryDomScraping(browser, onProgress) {
  ...
  try {
    ...
    await page.goto('https://www.bestbuy.ca/en-ca/deals', {
      waitUntil: 'domcontentloaded',
      timeout: 30000,
    });
    ...
    for (const product of products) {
      try {
        ...
        const deal = {
          id: slugify(`bestbuy-${product.name}`),
          store: STORE_NAME,
          storeKey: STORE_KEY,
          name: product.name,
          url: product.url,

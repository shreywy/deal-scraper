...
const STORE_NAME = 'Best Buy Outlet';
const STORE_KEY = 'bestbuyoutlet';
const CURRENCY = 'CAD';
...
/**
 * Scrapes deals from Best Buy Outlet (open box, refurbished)
...
async function scrape(browser, onProgress = () => {}) {
  ...
  try {
    onProgress('Best Buy Outlet: trying XHR interception…');
    const xhrDeals = await tryXhrInterception(browser, onProgress);
    if (xhrDeals && xhrDeals.length > 0) {
      deals.push(...xhrDeals);
      onProgress(`Best Buy Outlet: found ${deals.length} deals via XHR`);
      return deals;
    }

    // Fallback to DOM if XHR fails
    onProgress('Best Buy Outlet: trying DOM scraping…');
    const domDeals = await tryDomScraping(browser, onProgress);
    if (domDeals && domDeals.length > 0) {
      deals.push(...domDeals);
      onProgress(`Best Buy Outlet: found ${deals.length} deals via DOM`);
      return deals;
    }

    onProgress('Best Buy Outlet: no deals found');
    return deals;

  } catch (error) {
    onProgress(`Best Buy Outlet: error — ${error.message}`);
...
async function tryXhrInterception(browser, onProgress) {
  ...
  try {
        ...
        if (contentType.includes('json') &&
          ...
          try {
            ...
            if (Array.isArray(products) && products.length > 0) {
              ...
              onProgress(`Best Buy Outlet XHR: intercepted ${products.length} products (total: ${interceptedProducts.length})`);
            }
          } catch (jsonError) {
    ...
    // Navigate to open box page
    await page.goto('https://www.bestbuy.ca/en-ca/brand/open-box', {
  ...
  try {
    ...
    const products = await page.evaluate(() => {
      ...
      return cards.map(card => {
        try {
          // Find product name
          const nameSelectors = ['.productItemName', '[class*="productName"]', 'h4', 'h3', '.sku-header'];
          let name = '';
          ...
          // Find sale price
          const salePriceSelectors = ['.style-module_screenReaderOnly__4QmbS', '[class*="salePrice"]', '[class*="currentPrice"]', '.priceValue'];
          let salePrice = null;
          ...
          // Find regular price
          const regPriceSelectors = ['.style-module_salePrice__1AlPw', '[class*="regularPrice"]', '[class*="wasPrice"]', 'del', 's'];
    ...
    for (const product of products) {
      try {
        ...
        const deal = {
          id: slugify(`bestbuyoutlet-${product.name}`),
          store: STORE_NAME,
          storeKey: STORE_KEY,
          name: product.name,
          url: product.url,
...
/**
 * Parse Best Buy Outlet products from API response
 */
  ...
  for (const product of products) {
    try {
      ...
      if (image && !image.startsWith('http')) {
        image = `https://www.bestbuy.ca${image}`;
      }

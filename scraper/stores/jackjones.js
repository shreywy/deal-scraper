...
// Jack & Jones Canada sale page
const SALE_URL = 'https://www.jackjones.com/en-ca/t/sale-men';
...
async function scrape(browser, onProgress = () => {}) {
  ...
  try {
    onProgress('Jack & Jones: navigating to sale page…');
    ...
    onProgress('Jack & Jones: loading products…');
    for (let i = 0; i < 5; i++) {
    ...
    onProgress('Jack & Jones: extracting deals…');
    const deals = await page.evaluate(({ storeName, storeKey }) => {
      ...
      return [...cards].map(card => {
        ...
        const imgEl = card.querySelector('img[src*="jackjones"], img[src*="jj"], img');
    ...
    onProgress(`Jack & Jones: found ${tagged.length} deals`);
    return tagged;

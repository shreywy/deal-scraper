...
// Lenovo Canada outlet and deals scraper
// Uses XHR intercept to capture product API responses + DOM fallback

const URLS = [
  'https://www.lenovo.com/ca/en/outletsales/',
  'https://www.lenovo.com/ca/en/deals/',
];
...
async function scrape(browser, onProgress = () => {}) {
  ...
  for (const url of URLS) {
    onProgress(`Lenovo: loading ${url.includes('outlet') ? 'outlet' : 'deals'} page…`);
    ...
    try {
      ...
      onProgress(`Lenovo: error on ${url} — ${err.message}`);
    } finally {
      await page.close();
    }
  }
  ...
  await context.close();
  onProgress(`Lenovo: found ${allDeals.length} deals total`);
  return allDeals;
}
...
async function scrapePage(page, url, onProgress) {
  ...
  page.on('response', async resp => {
    ...
    if ((respUrl.includes('lenovo.com') || respUrl.includes('lenovoapi')) &&
        resp.headers()['content-type']?.includes('json')) {
      try {
        const data = await resp.json();
        const prods = data?.body?.data?.products || data?.products || data?.data?.products || data?.items || [];
  ...
  try {
    await page.click('#truste-consent-button, [class*="cookie-accept"]', { timeout: 4000 });
  } catch (_) {}
  ...
  try {
    ...
    onProgress('Lenovo: no product grid found, trying scroll anyway…');
  }
  ...
  if (intercepted.length > 0) {
    onProgress(`Lenovo: processing ${intercepted.length} intercepted products`);
    const deals = intercepted.map(p => {
      const price = parseFloat(p.currentPrice || p.price || p.salePrice || 0);
      const originalPrice = parseFloat(p.listPrice || p.originalPrice || p.msrp || 0);
      ...
      const image = p.image || p.imageUrl || p.thumbnail || p.images?.[0]?.url || '';
      ...
      return {
        ...
        storeKey: STORE_KEY,
        name,
        url: productUrl.startsWith('http') ? productUrl : `https://www.lenovo.com${productUrl}`,
        image: image.startsWith('http') ? image : `https://www.lenovo.com${image}`,
        price,
        originalPrice,
        discount,
        category,
  ...
  onProgress('Lenovo: using DOM fallback');
  const deals = await page.evaluate(({ storeName, storeKey }) => {
    ...
    return cards.map(card => {
      ...
      const nameEl = card.querySelector('[class*="productName"], [class*="product-name"], h2, h3, h4');
      const salePriceEl = card.querySelector('[class*="salePrice"], [class*="sale-price"], [class*="currentPrice"], [class*="promo"]');
      const origPriceEl = card.querySelector('[class*="listPrice"], [class*="originalPrice"], [class*="was-price"], s, del, strike');
...
async function loadProducts(page, onProgress) {
  ...
  while (round < MAX_ROUNDS) {
    ...
    try {
      ...
      await btn.click();
      round++;
      onProgress(`Lenovo: loading more products (page ${round + 1})…`);

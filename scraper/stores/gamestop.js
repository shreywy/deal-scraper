...
const STORE_NAME = 'GameStop CA';
const STORE_KEY = 'gamestop';
const CURRENCY = 'CAD';
...
/**
 * Scrapes deals from GameStop Canada
...
async function scrape(browser, onProgress = () => {}) {
  ...
  try {
    onProgress('GameStop CA: trying XHR interception…');
    const xhrDeals = await tryXhrInterception(browser, onProgress);
    if (xhrDeals && xhrDeals.length > 0) {
      deals.push(...xhrDeals);
      onProgress(`GameStop CA: found ${deals.length} deals via XHR`);
      return deals;
    }

    // Fallback to DOM if XHR fails
    onProgress('GameStop CA: trying DOM scraping…');
    const domDeals = await tryDomScraping(browser, onProgress);
    if (domDeals && domDeals.length > 0) {
      deals.push(...domDeals);
      onProgress(`GameStop CA: found ${deals.length} deals via DOM`);
      return deals;
    }

    onProgress('GameStop CA: no deals found');
    return deals;

  } catch (error) {
    onProgress(`GameStop CA: error — ${error.message}`);
...
async function tryXhrInterception(browser, onProgress) {
  ...
  try {
        ...
        if (contentType.includes('json') &&
          ...
          try {
            ...
            if (Array.isArray(products) && products.length > 0) {
              ...
              onProgress(`GameStop CA XHR: intercepted ${products.length} products (total: ${interceptedProducts.length})`);
            }
          } catch (jsonError) {
    ...
    await page.goto('https://www.gamestop.ca/en/Browse/Games/pc-mac?sortBy=PriceLow&filterBy=Deal', {
  ...
  try {
    ...
    for (const product of products) {
      try {
        ...
        const deal = {
          id: slugify(`gamestop-${product.name}`),
          store: STORE_NAME,
          storeKey: STORE_KEY,
          name: product.name,
          url: product.url,
...
/**
 * Parse GameStop products from API response
 */
  ...
  for (const product of products) {
    try {
      ...
      if (image && !image.startsWith('http')) {
        image = `https://www.gamestop.ca${image}`;
      }

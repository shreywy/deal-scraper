...
// Lacoste Canada sale page
const SALE_URL = 'https://www.lacoste.com/en-ca/t/men-sale';
...
async function scrape(browser, onProgress = () => {}) {
  ...
  try {
    onProgress('Lacoste: navigating to sale page…');
    ...
    onProgress('Lacoste: loading products…');
    for (let i = 0; i < 5; i++) {
    ...
    onProgress('Lacoste: extracting deals…');
    const deals = await page.evaluate(({ storeName, storeKey }) => {
      ...
      return [...cards].map(card => {
        ...
        const imgEl = card.querySelector('img[src*="lacoste"]');
    ...
    onProgress(`Lacoste: found ${tagged.length} deals`);
    return tagged;
